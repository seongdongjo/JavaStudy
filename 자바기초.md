...
또한 사용자가 파라미터에 옳지 않은 값을 줄수도 있으니 이를 캐치하여 잘 분기 처리하여야 한다.
...
public E remove(int index) {
	...
	for(int i=index; i<size-1; i++) {
		elementData[i] = elementData[i+1]; //빠진곳 채우기
...
/**
* session이 끊어졌을 떄를 처리하기 위해 사용
* static메소드에서는 static 만 사용하므로 static으로 선언한다.
*/
public class LoginManager implements HttpSessionBindingListener {
	...
	* 싱글톤 패턴 사용
	*/
	public static synchronized LoginManager getInstatnce() {
	...
	/*
	* 이 메소드는 세션이 연결됐을 때 호출된다.(session.setAttribute("login",this))
	* Hashtable에 세션과 접속자 아이디를 저장한다.
	*/
	public valueBound(HttpSessionBindingEvent event) {
		//session값을 static해시테이블(loginUser)에 put한다
	...
	/*
	* 이 메소드는 세션이 끊겼을 때 호출된다.(invalidate)
	...
	public void removeSession(String userId) {
		...
		while(e.hasMoreElements()) {
			...
			if(loginUsers.get(session).equals(userId)) { //
				//세션이 invalidate 될때 HttpSessionBindingListener를
				//구현하는 클래스의 valueUnbound() 함수가 호출된다.
				session.invalidate();
			}
		}
	}
	...
	*/
	public boolean isValid(String userId, String userPw) {
		/*
		* 이 부분에는 DB를 접속에서 일치여부 확인 해서 true, false 반환
		*/
		return true;
	}

	/*
	* 해당 아이디의 동시 사용을 막기 위해서
	* 이미 사용중인 아이디인지를 확인
	*/
	...
	/*
	* 로그인을 완료한 사용자의 아이디를 세션에 저장하는 메서드
	*/
	public void setSession(HttpSession session, String userId) {
		//이순간에 Session Binding 이벤트가 일어나는 시점
	...
	*/
	public String getUserId(HttpSession session) {
		return (String)loginUsers.get(session); //접속자아이디 반환
	}
