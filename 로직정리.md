...
<select id="selectSlipMng" parameterType="TRSlipMngVO" resultType="egovMap">
	SELECT
	   //현재 createQcell에는 없지만 행추가시 왜냐하면 setCellDataEx를 하기 위해서는 해당컬럼(connTime)이 있어야 set이 된다.  
...
function notRegistjunpyoPopup(obj, row, col) {
		...
		if (qcell.getRowData(row).accoCode == null
				|| qcell.getRowData(row).accoCode == '') {
			alert('계정과목은 빈 값이 될 수 없습니다.')
			return false;
		}
		//connYn=Y 이면 즉, 이미 미처리전표팝업에서 체크 후 확인을 했을 때
		if (qcell.getRowData(row).connYn == 'Y') {
			alert('이미 미처리전표에서 연계하였습니다.\n삭제 후 다시 추가해주세요.');
			return false;
		}
		...
		if (qcell.getRowData(row).slipNo == null || qcell.getRowData(row).slipNo == '') {
			...
			junpyoPopup(qcell.getRowData(row).chadaeDiv,
					qcell.getRowData(row).accoCode,
					qcell.getRowData(row).custCode1)
		}
	}
...
function junpyoPopup(chadae,acco,cust) {
	...
	if(chadae == '1') { //그리드가 차변일때 미처리전표조회 시 대변으로 조회해야됨(반대로)
			document.getElementById('chadaeDiv').value = '2'
	}
	if(chadae == '2') {
		document.getElementById('chadaeDiv').value = '1'
	}
	document.getElementById('disposeMoney').value = '' //입력항목에 처리금액(전에 값을 지우기위해)
	document.getElementById('accoCode').value = acco //히든컬럼에 받은 인자인 acco를 accoCode에 저장된다.
	document.getElementById('custCode').value = cust //히든컬럼에 받은 인자인 cust를 custCode에 저장된다.
...
//즉, 미처리전표 조회그리드에 createQcell에 slipAmt 등 지정안해도 그리드에 담겨져있다. 안보여도
...
<select id="selectJunpyoPopupMng" parameterType="TRSlipMngVO" resultType="egovMap">
		SELECT 
 		...
 		,A.CUST_CODE
 		,B.ACCO_NAME
 		,C.CUST_NAME		//차변(1)일 경우 차변금액, 대변(2)일때 대변금액을 TRSLIP에서 찾아서 SLIP_AMT로 지정한다. 이걸 나중에 TRSLTP에 넣을것이다(미처리전표팝업 시 그리드에 숨겨놓음)
		,CASE WHEN #{chadaeDiv} = '1' THEN A.DEBTOR_AMT ELSE A.CREDIT_AMT END AS SLIP_AMT
							...
							//왜 빼냐하면 이미 연계한 금액은 빼야함. 연계한 금액만큼을 또 쓸 수 없으니까
...
- 팝업에서 연계하려는 전표들을 체크박스로 선택하고(여러개 선택가능) 확인버튼 누르면 
  ...
  단, 전표그리드에 입력했던 차변,대변금액은 덮어써진다(차대구분에 따라서).

분할선택 시: 
- 한건만 선택가능하며, 얼만큼 분할할지 처리금액을 입려항목에 적는데
  분할하려는 금액보다 같거나 크면 안된다. 확인 버튼 누르면!!!!
  차변,대변에 따라서(전표등록관리에서 차대구분) 금액이 알아서 전표 그리드에 들어간다. 
...
function junpyoCreate() {
	...
	disposeMoney = document.getElementById('disposeMoney').value //입력한 처리금액
	totAmt = 0;
	if(chks.length < 1){
		return false;
	} 
	...
	/*2. 임시테이블(TRSLTP) 데이터 삭제(connTime, 등록ID를 보고)**/
	//즉, 같은 행에 연계처리를 했는데 또 연계처리하려할때 지운다.(임시테이블에서)
	deleteTrsltp(qcell.getCellDataEx(qcell.getIdx('row'),'connTime'))

	//위에서 삭제 후 임시테이블에 넣기
	//임시테이블에 넣기 전에 임시테이블에 해당 데이터가 있는지 확인이 필요
	for(var i=0; i<=chks.length-1; i++) {//TRSLTP에서 키인 SLIP_NO, SLIP_SEQ가 충돌하는지 확인(다른유저가 쓸 수도있으니)
	...
	//여기까지왔다면 임시테이블에 겹치는게 없다는 말이다.
		for (var i = 0; i <= chks.length - 1; i++) {
			if (chks.length > 1) { //미처리전표팝업에서 여러출 체크시 -> 여러줄 체크의 의미는 전체,분할중에 전체를 선택했다는 의미이다.
				if (i==0) { //첫줄은 이미 행추가시에 필요한값을 미리 넣었으니까 거래처코드만 가져온다.(즉, 확인누르면 체크한 미처리전표 팝업의 거래처를 전표그리드의 거래처에 넣음)
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'custCode1',junpyoQcell.getCellDataEx(chks[i], 'custCode'))
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'custName1',junpyoQcell.getCellDataEx(chks[i], 'custName'))
				}
				//미처리전표 팝업에서 두줄이상 체크시에만 행 추가!!!! 및 기본값세팅
				if (i > 0) { //첫줄은 행추가로 추가 및 값세팅이 되니까 아래 값세팅은 두번째줄부터 세팅해주면된다.
					qcell.insertRow(qcell.getIdx('row') + i) //둘째줄부터 기본세팅 일단 금액빼고
					sleep(1000);//행추가시 시간이 겹치는 현상으로 인한 추가
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'custCode1',junpyoQcell.getCellDataEx(chks[i], 'custCode'))
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'custName1',junpyoQcell.getCellDataEx(chks[i], 'custName'))
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'slipDate', getToday2().replace(/-/g, ""))
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'connTime', getyyyymmddhhmmss())
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'connYn', 'N')
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'accoCode',qcell.getCellDataEx(qcell.getIdx('row'),'accoCode'))
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'accoName',
					qcell.getCellDataEx(qcell.getIdx('row'),'accoName'))
					qcell.setCellDataEx(qcell.getIdx('row') + i,'debtorAmt', 0)
					qcell.setCellDataEx(qcell.getIdx('row') + i,'creditAmt', 0)
					//미처리전표팝업 조회 대상의 대체,차변구분을 가져와서 복사해준다. 
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'slipDiv',qcell.getCellDataEx(qcell.getIdx('row'),'slipDiv'))
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'chadaeDiv',
					qcell.getCellDataEx(qcell.getIdx('row'),'chadaeDiv'))
					//전줄의 차대구분에 따라 차변,대변금액 입력가능여부
					if (qcell.getCellDataEx(qcell.getIdx('row'), 'chadaeDiv') == '1') {
						qcell.setCellDisable(qcell.getIdx('row') + i, 16, true);
					} else {
						qcell.setCellDisable(qcell.getIdx('row') + i, 15,false);
					}
				}
				//행추가 시에 생긴 connTime을 미처리전표팝업 한줄에 등록(임시테이블에 넣기위해)
				junpyoQcell.setCellDataEx(chks[i], 'accoTime', qcell.getCellDataEx(qcell.getIdx('row') + i, 'connTime'))
				junpyoQcell.setCellDataEx(chks[i],'slipYn',$('input[name=dispose]:checked').val() == 'A' ? 'Y' : 'N')
				//여기 분기문은 여러개 체크를 했을 때니까(위에 if문) 분할은 해당사항이 안되서(분할은 하나밖에 체크못하니까) 
				//document.getElementById('disposeMoney').value (입력한처리금액)을 구해서 junpyoQcell의 slipAmt에 넣을 필요가 없다. 
				//그냥 junpyoQcell목록에 나와있는 금액을 체크한 그대로 임시테이블에 넣으면 된다.
				insertTrsltp(junpyoQcell.getRowData(chks[i]));

				//전체이면서 차변일때(여러줄 체크시)
				if ($('input[name=dispose]:checked').val() == 'A' && qcell.getCellDataEx(qcell.getIdx('row'), 'chadaeDiv') == '1') {
					//여러줄 체크했을 때는 전표금액말고 잔액으로 전표그리드에 넣으면 된다.(for문으로 행추가한 로우에 각각 들어가면된다.)   
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'debtorAmt',junpyoQcell.getCellDataEx(chks[i], 'balanceAmt'))
				} else if ($('input[name=dispose]:checked').val() == 'A' && qcell.getCellDataEx(qcell.getIdx('row'), 'chadaeDiv') == '2') { //전체이면서 대변일때
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'creditAmt',junpyoQcell.getCellDataEx(chks[i], 'balanceAmt'))
				}

				//connYn에 대입(if문에 따라)
				//A: 전체이면서 체크한잔액이 balanceAmt가 0원이면 N처리
				if ($('input[name=dispose]:checked').val() == 'A'&& junpyoQcell.getCellDataEx(chks[i], 'balanceAmt') == 0) {
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'connYn', 'N')
					//goToCell(qcell.getIdx('row'),3)
					//그외는 Y로 넣기
				} else {
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'connYn', 'Y')
					//goToCell(qcell.getIdx('row'),3)
				}

			} else { //한줄 체크시에!!!!!
				//추가한 행에 connTime을 미처리전표팝업 한줄에 등록(임시테이블에 넣기위해)
				...
				//전체,분할이면서 차변,대변일때
				if ($('input[name=dispose]:checked').val() == 'A' && qcell.getCellDataEx(qcell.getIdx('row'), 'chadaeDiv') == '1') {
					qcell.setCellDataEx(qcell.getIdx('row'), 'debtorAmt',junpyoQcell.getCellDataEx(chks[i], 'balanceAmt'))
					qcell.setCellDataEx(qcell.getIdx('row'), 'custCode1',junpyoQcell.getCellDataEx(chks[i], 'custCode'))
					qcell.setCellDataEx(qcell.getIdx('row'), 'custName1',junpyoQcell.getCellDataEx(chks[i], 'custName'))
					insertTrsltp(junpyoQcell.getRowData(chks[i])); //미처리전표에서 체크한 한건만 임시테이블에 넣는다.
				} else if ($('input[name=dispose]:checked').val() == 'A' && qcell.getCellDataEx(qcell.getIdx('row'), 'chadaeDiv') == '2') {
					qcell.setCellDataEx(qcell.getIdx('row'), 'creditAmt',junpyoQcell.getCellDataEx(chks[i], 'balanceAmt'))
					qcell.setCellDataEx(qcell.getIdx('row'), 'custCode1',junpyoQcell.getCellDataEx(chks[i], 'custCode'))
					qcell.setCellDataEx(qcell.getIdx('row'), 'custName1',junpyoQcell.getCellDataEx(chks[i], 'custName'))
					insertTrsltp(junpyoQcell.getRowData(chks[i])); //미처리전표에서 체크한 한건만 임시테이블에 넣는다.
				} else if ($('input[name=dispose]:checked').val() == 'D' && qcell.getCellDataEx(qcell.getIdx('row'), 'chadaeDiv') == '1') {
					//처리금액을 미처리그리드의 slipAmt에 넣음.(분할이니까 input값을 가져와야함)
					//그래서 위와 다르게 junpyoQcell.setCellDataEx를 해서 입력한 처리금액을 넣은것이다. 전체는 그냥 체크한 금액 전체넣으면되서 안했던거고
					junpyoQcell.setCellDataEx(chks[i], 'slipAmt', parseInt(uncomma(document.getElementById('disposeMoney').value)))
					qcell.setCellDataEx(qcell.getIdx('row'), 'debtorAmt',parseInt(uncomma(document.getElementById('disposeMoney').value)))
					qcell.setCellDataEx(qcell.getIdx('row'), 'custCode1',junpyoQcell.getCellDataEx(chks[i], 'custCode'))
					qcell.setCellDataEx(qcell.getIdx('row'), 'custName1',junpyoQcell.getCellDataEx(chks[i], 'custName'))
					insertTrsltp(junpyoQcell.getRowData(chks[i])); //한건만
				} else if ($('input[name=dispose]:checked').val() == 'D' && qcell.getCellDataEx(qcell.getIdx('row'), 'chadaeDiv') == '2') {
					junpyoQcell.setCellDataEx(chks[i], 'slipAmt', parseInt(uncomma(document.getElementById('disposeMoney').value)))
					qcell.setCellDataEx(qcell.getIdx('row'), 'creditAmt',parseInt(uncomma(document.getElementById('disposeMoney').value)))
					qcell.setCellDataEx(qcell.getIdx('row'), 'custCode1',junpyoQcell.getCellDataEx(chks[i], 'custCode'))
					qcell.setCellDataEx(qcell.getIdx('row'), 'custName1',junpyoQcell.getCellDataEx(chks[i], 'custName'))
					insertTrsltp(junpyoQcell.getRowData(chks[i])); //한건만
				}
				...
				//추후에 impl에서 전표등록을 하고 난 후에 if("Y".equals(gridI.getConnYn())) 해서 연계전표를 등록할 경우
				//임시테이블에서 slipMngDAO.selectTrsltp(gridI)을 통해 

				//분할이면서 0원일때
				if ($('input[name=dispose]:checked').val() == 'D' && document.getElementById('disposeMoney').value == '0') {
					qcell.setCellDataEx(qcell.getIdx('row'), 'connYn', 'N') //처음에는 확인누를때는 Y 단, 넘어오는 금액이 0원이면 N처리
					goToCell(qcell.getIdx('row'), 3)
					//전체이면서 한건 금액이 0원일때
				} else if ($('input[name=dispose]:checked').val() == 'A' && junpyoQcell.getCellDataEx(chks[i], 'balanceAmt') == 0) {
					qcell.setCellDataEx(qcell.getIdx('row'), 'connYn', 'N') //처음에는 확인누를때는 Y 단, 넘어오는 금액이 0원이면 N처리
					goToCell(qcell.getIdx('row'), 3)
					//그외는 Y로 넣기
				} else {
					qcell.setCellDataEx(qcell.getIdx('row'), 'connYn', 'Y') //처음에는 확인누를때는 Y 단, 넘어오는 금액이 0원이면 N처리
					goToCell(qcell.getIdx('row'), 3)
				}
			}
		}
...
function registBtn() {
	...
	if(checkRowIndex.length > 0) {
		...
		if(sumDeb != sumCre) {
			...
			$(".sumRows").text(sumDeb-sumCre + " 원");
			$(".sumDeb").text(sumDeb+" 원");
			$(".sumCre").text(sumCre+" 원");
			return false;
		}
		
		//전표등록처리
		document.getElementById('checkJunpyo').value = checkRowIndex
		insertTrslip()
...
SuppressWarnings({ "unchecked", "unused" })
	...
	public void insertRegisteredSlipMng(Map<String, Object> vo) throws Exception {
		...
		String strParam = (String) vo.get("param"); //화면상에 form -> name 값 사용
		paramMap = fn.jsonToMap(strParam);
		String checkJunpyo = (String) vo.get("checkJunpyo");
		...
		String[] checkJunpyoList;
		checkJunpyoList = checkJunpyo.split(",");
		...
		//체크한 값 하나하나 반복(체크한 전표리스트개수까지만 일단 반복 - 이거안하면 체크안한 추가로우까지 잡혀버림)
		for(int i=0; i < list.size(); i++) {
			gridI = objectMapper2.convertValue(fn.mapToVo(list,i), TRSlipMngVO.class);
			sRow = objectMapper2.convertValue(fn.rowsMapToVo(list,i), int.class);
			for(int j = 0; j<checkJunpyoList.length; j++) {
				if(Integer.parseInt(checkJunpyoList[j]) == sRow) { //체크한 로우와 추가한 로우가 같을때만 -이거안하면 체크안한 추가로우까지 잡혀버림
					...
					if("Y".equals(gridI.getConnYn())) { //연계전표를 전표등록할 경우
						...
						for(int k = 0; k<gridList.size(); k++) {
							gridList.get(k).put("newSlipNo", newSlipNo);
							gridList.get(k).put("newSlipSeq", newSlipSeq);
							slipMngDAO.updateLinkTrslip(gridList.get(k));
						}
					}
				}
			}
...
//쿼리
<!--새 전표등록을 먼저 위에서 하고 이후에 연계체크(connYn)가 되어있으면 
	위에서 등록한 새전표번호를 원전표(미처리전표)에다가 넣는다 세개 체크하면 세개의 각 미처리연계전표에 connno,seq,amt 에 들어간다.
-->
<update id="updateLinkTrslip" parameterType="map">
		UPDATE TRSLIP
			SET
				SLIP_YN = CASE WHEN #{slipYn} = 'Y' THEN 'Y' ELSE 'N' END
				,CONN_SLIP_NO = CASE WHEN #{slipConnSeq} = '1' THEN #{newSlipNo} ELSE ''END
				,CONN_SLIP_SEQ = CASE WHEN #{slipConnSeq} = '1' THEN #{newSlipSeq} ELSE '' END
				,CONN_SLIP_AMT = CASE WHEN #{slipConnSeq} = '1' THEN #{slipAmt} ELSE 0 END
				,CONN_SLIP_NO1 = CASE WHEN #{slipConnSeq} = '2' THEN #{newSlipNo} ELSE '' END
				,CONN_SLIP_SEQ1 = CASE WHEN #{slipConnSeq} = '2' THEN #{newSlipSeq} ELSE '' END
				,CONN_SLIP_AMT1 = CASE WHEN #{slipConnSeq} = '2' THEN #{slipAmt} ELSE 0 END
				,CONN_SLIP_NO2 = CASE WHEN #{slipConnSeq} = '3' THEN #{newSlipNo} ELSE '' END
				,CONN_SLIP_SEQ2 = CASE WHEN #{slipConnSeq} = '3' THEN #{newSlipSeq} ELSE '' END
				,CONN_SLIP_AMT2 = CASE WHEN #{slipConnSeq} = '3' THEN #{slipAmt} ELSE 0 END
			WHERE
				SLIP_NO = #{slipNo} AND SLIP_SEQ = #{slipSeq} //여기서 slipNo는 Trsltp에 있는 체크한 미처리 전표이다.
</update>
...
//[행삭제]
function delBtn() {
		var delJunpyo = new Array();
		var alertMsg = ""; //삭제 시에 연계전표 메시지를 출력하기 위해서
		var qcell = QCELL.getInstance('qcell'), 
		...
		for(var k = 0; k<=chks.length-1; k++) {
			if(qcell.getRowState(chks[k] == 'i') || qcell.getRowState(chks[k] == 'u')) {
				alert('추가와 수정은 상태표시를 더블클릭하여 삭제 및 수정변화 표시를 지울 수 있습니다.')
				return false;
			}
		}
		...
		for (var i = 0; i <= chks.length - 1; i++) { //체크한 길이 만큼 반복
			if (qcell.getCellDataEx(chks[i], "slipFinishYn") == 'Y') {
				alert('전표번호[' + qcell.getCellDataEx(chks[i], "slipNo")
						+ ']에 대한 일마감처리가 되었습니다.\n삭제할 수 없습니다.');
				return false;
			}
			/*3. 연계처리된 전표번호 존재여부 check**/
			var connSlipData = checkConnSlipNo(qcell.getCellDataEx(chks[i], "slipNo"))  //체크한 전표 하나씩 넘긴다.
			var alertMessage = '['
			if(connSlipData.connSlipno != '') {
				for(var j = 0; j<connSlipData.connSlipNoList.length; j++) { //하나씩 
					if(j == connSlipData.connSlipNoList.length-1) {
						alertMessage += connSlipData.connSlipNoList[j].connSlipNo + ']'
					}else {
						alertMessage += connSlipData.connSlipNoList[j].connSlipNo +','
					}
				}
			...
			/* if (parseInt(checkMaxSlipSeq(
					qcell.getCellDataEx(chks[i], "slipNo"), qcell
							.getCellDataEx(chks[i], "slipSeq"))) > 0) {
				alert('전표번호[' + qcell.getCellDataEx(chks[i], "slipNo")
						+ ']에 대한 전표순번['
						+ qcell.getCellDataEx(chks[i], "slipSeq")
						+ ']보다 큰 순번의 Data가 존재합니다.\n삭제할 수 없습니다.')
				return false;
			} */
		}
		...
		for (var i = 0; i <= chks.length - 1; i++) {
			//체크한 전표번호 배열에 추가
			delJunpyo.push(qcell.getCellDataEx(chks[i],"slipNo"))
			//if(qcell.getCellDataEx(chks[i], "slipNo") == qcell.getCellData())
			//qcell.setRowState(chks[i], "d");
		}
		//중복제거
		delJunpyo = [... new Set(delJunpyo)];
		document.getElementById("delJunpyo").value = delJunpyo
		if(confirm("["+ delJunpyo+"] 에 해당하는 전표를 일괄삭제 하시겠습니까?")) {
			deleteTrslip();
		}
...
```
//컨트롤러
 /** 삭제하려는 전표에서 연계처리된 전표가 있는지 check*/
    @NoLogging
    @RequestMapping(value="/transcls/cost/slipMng/checkConnSlipNo.do")
    public @ResponseBody HashMap<String, Object> checkConnSlipNo(@ModelAttribute("tRSlipMngVO") TRSlipMngVO tRSlipMngVO, ModelMap model) throws Exception {
    	
    	String connSlipno = null;
    	LoginVO user = (LoginVO)EgovUserDetailsHelper.getAuthenticatedUser();
    	List<Map<String,String>> connSlipNoList = new ArrayList<Map<String,String>>();
    	HashMap<String, Object> resMap = new HashMap<String, Object>();
		try {										   //VO에는 slipNo만 담긴다.
			connSlipno = slipMngService.checkConnSlipNo(tRSlipMngVO,connSlipNoList); //아래 impl 참조
			 
			Function fn = new Function(); 
			JSONArray jsonArr = fn.listToJson(connSlipNoList);
			resMap.put("connSlipno", connSlipno);
			resMap.put("connSlipNoList", jsonArr);
		}catch(Exception e) {
			e.printStackTrace();
		}
...
/**
  * 전표등록관리 > 연계처리전표 check
	public String checkConnSlipNo(TRSlipMngVO tRSlipMngVO,List<Map<String,String>> connSlipNoList) throws Exception {
		String connSlipNo = null;
		connSlipNo = slipMngDAO.checkConnSlipNo(tRSlipMngVO); //일단 먼저 연계처리 데이터가 있는지 먼저 체크
		if(connSlipNo == null || "".equals(connSlipNo)) {
			
		}else { //연계된 데이터가 있다면 리스트형식으로 받는다.
			for(int i = 0; i<slipMngDAO.selectConnSlipNoList(tRSlipMngVO).size(); i++) {
				connSlipNoList.add(slipMngDAO.selectConnSlipNoList(tRSlipMngVO).get(i)); //VO를 던저주면 
			}
		}
		return connSlipNo;
	}
...
```
//쿼리
<!--일전표마감 count조회(지금안씀) 그냥 쿼리 공부하기(SLIP_SEQ = #{slipSeq} + 1을 할수 있다) -->
...
```
//여기서 
<select id="selectConnSlipNoList" parameterType="TRSlipMngVO" resultType="egovMap">
	<![CDATA[
		SELECT 
			CONN_SLIP_NO 
		FROM TRSLIP WHERE SLIP_NO = #{slipNo} AND CONN_SLIP_NO <> ''
			UNION 
		SELECT 
			CONN_SLIP_NO1 
		FROM TRSLIP WHERE SLIP_NO = #{slipNo} AND CONN_SLIP_NO1 <> ''
			UNION
		SELECT 
			CONN_SLIP_NO2
		FROM TRSLIP WHERE SLIP_NO = #{slipNo} AND CONN_SLIP_NO2 <> ''
	]]>
    </select>
```
```
...
//impl에서 차대구분이 2(대변)이면서 ACCO_CODE가 301(외상매입금)인 경우
		...
		if(confirm('전표일['+document.getElementById('schSlipDate').value+']에 해당하는\n'+taxDiv+'세금계산서에 대한 자동전표생성처리 하시겠습니까?')) {
			if(checkRegistered(document.getElementById('schSlipDate').value.replaceAll('-','')) > 0) { //해당날짜에 이미 마감되어있는지 체크
				...
				if(checkMaecl(document.getElementById('schSlipDate').value.replaceAll('-','')) <= 0) {
					...
					//전표생성l
					createMaecl(document.getElementById('schSlipDate').value.repaceAll('-',''))
				}
...
function createMaecl(schSlipDate) { //인자는 20231123 형태로온다.
	qcell.showProgress();
	$.ajax({
		url: "/transcls/cost/slipMng/selectMaeclSlipMng.do", //해당 컨트롤러에서 전표를 발생시키고 조회한다.
		data: {
...
List<TRCtxMngVO> gridList = slipMngService.selectMaeclSlipMng(tRSlipMngVO); //여기 VO에는 입력한 세금계산서처리 전표일자가 담긴다.
<!--전표미발행 매출세금계산서 조회
	여기서 내가 테스트 할 때는 TRCTAX에서 그냥 행복사해서 TAX_FINISH_YN='N' 으로 데이터를 만들어서 테스트 했다.-->
...
//위에서 TRCTAX(매출세금계산서)에서 조회한 값을(gridList) 아래에 던진다.(컨트롤러)
//그러면 gridList 안에는 입력한 세금계산서처리날짜에 TAX_FINISH_YN='N'인것을 가져온다(JIJUM,TAX_NO,...)
slipMngService.createMaeclSlipMng(gridList,tRSlipMngVO.getSchSlipDate());

//createMaeclSlipMng (Impl)에서 보면 새 전표번호를 생성해야하는데
//위에서 조회한 건수 만큼 반복해서 전표를 생성한다.
public void createMaeclSlipMng(List<TRCtxMngVO> gridList, String schSlipDate) {
	...
	for(int i = 0; i<gridList.size(); i++) {
		...
		slipMngDAO.createMaeclSlipMng(vo); //아래 참조
	} 
}
...
#)slipMngDAO.createMaeclSlipMng
public void createMaeclSlipMng(TRCtxMngVO vo) throws Exception {
	insert("slipMngDAO.createMaeclSlipMng",vo);
	insert("slipMngDAO.createMaeclSlipMng2",vo);
	insert("slipMngDAO.createMaeclSlipMng3",vo);
	update("slipMngDAO.updateMaeclSlipMng",vo);
}


//위에서 구한 데이터를(TRCTAX에서 select한 값과 안에 넣은 새로운전표번호) 던지는 이유는 전표DATA를 3개 생성해야하는데 아래에서 쓰인다.(전표번호 하나에 대해서 전표순번3개)
매출전표를 생성할때는
...
매입전표도 마찬가지 생성할 때는 아래같은 사항으로 값만 넣으면 된다.
...
function notRegistjunpyoPopup(obj, row, col) {
		...
		//connYn=Y 이면 즉, 이미 미처리전표팝업에서 체크 후 확인을 했을 때
		//즉, 팝업에서 확인을 누르면 해당 추가한 로우에 Y가 들어간다. 그래서 또 돋보기를 누르면 메시지가 뜬다.
		if (qcell.getRowData(row).connYn == 'Y') {
			alert('이미 미처리전표에서 연계하였습니다.\n삭제 후 다시 추가해주세요.');
			return false;
		}
...
//미처리 그리드 팝업을 띄운다.
function junpyoPopup(chadae, acco, cust) { //팝업을 호출한 로우의 차대구분,계정과목,거래처를 가져온다
		var date = new Date();
		var today = getToday2();
		var junSlipDateFr = date.getFullYear() - 2 + "-01-01"; //2년전날짜 
		var junSlipDateTo = getToday2(); //오늘날짜
		if(chadae == '1') {
			document.getElementById('chadaeDiv').value = '2'
		}
		if(chadae == '2') {
		...
		document.getElementById('disposeMoney').value = ''
		document.getElementById('accoCode').value = acco
		document.getElementById('custCode').value = cust
		document.getElementById('junSlipDateFr').value = junSlipDateFr
		document.getElementById('junSlipDateTo').value = junSlipDateTo
		var formData = decodeURIComponent($('#junpyoSearchForm').serialize());
		let junpyoQcell;
		$.ajax({
			url : "/transcls/cost/slipMng/selectJunpyoPopupMng.do",
			...
			success : function(data) {
				...
				if (typeof junpyoQcell === "undefined"
						|| typeof junpyoQcell !== "object") {
					QCELL.create({
						id : "junpyoQcell",
						parentid : "junpyoSheetarea",
						rowheaders : [ "checkbox", "sequence" ], // state : 행추가, 데이터변경, 행삭제 시 해당 rowheader 영역에 신호등 표현
						...
						columns : [ 
						   ...
						   ,{width : "11%", key : "slipDate", title : [ "전표일자" ], styleclassname : {data : "align-center"}}
						   ,{width : "12%", key : "accoName", title : [ "계정과목" ], styleclassname : {data : "align-center"}}
						   ,{width : "20%", key : "custName", title : [ "거래처" ], search: true, styleclassname : {data : "align-center"}}
						   ,{width : "12%", key : "slipAmt", title : [ "전표금액" ], resize : true, styleclassname : {data : "align-right"}, options : {format : {type : "number", rule : "#,###"}}}
						   ,{width : "12%", key : "balanceAmt", title : [ "잔액" ], resize : true, styleclassname : {data : "align-right"}, options : {format : {type : "number", rule : "#,###"}}}
						   ,{width : "20%", key : "remark", title : [ "적요" ], styleclassname : {data : "align-center"}}
						   , 
...
```
```
- 위의 ajax에서 호출한 미처리 전표 팝업 데이터 호출
- /transcls/cost/slipMng/selectJunpyoPopupMng.do 에대한쿼리이다.
SELECT 
		 A.SLIP_NO
 		,A.SLIP_SEQ
 		,A.SLIP_DATE
 		...
 		,B.ACCO_NAME
 		,C.CUST_NAME		
		,CASE WHEN '2' = '1' THEN A.DEBTOR_AMT ELSE A.CREDIT_AMT END AS SLIP_AMT
		,CASE WHEN '2' = '1' THEN A.DEBTOR_AMT ELSE A.CREDIT_AMT END - (A.CONN_SLIP_AMT + A.CONN_SLIP_AMT1 + A.CONN_SLIP_AMT2) AS BALANCE_AMT	
		,CASE WHEN A.CONN_SLIP_NO = '' THEN '1' ELSE (CASE WHEN A.CONN_SLIP_NO1 = '' THEN '2' ELSE '3' END) END AS CONN_SLIP_DIV
	FROM TRSLIP A
		LEFT OUTER JOIN TRACCO B ON (A.ACCO_CODE = B.ACCO_CODE)
		LEFT OUTER JOIN TRCUTA C ON (A.CUST_CODE = C.CUST_CODE)
		LEFT OUTER JOIN TRSLTP D ON (A.SLIP_NO = D.SLIP_NO AND A.SLIP_SEQ = D.SLIP_SEQ) //이부분을 추가함(임시테이블)
	WHERE A.SLIP_DATE BETWEEN REPLACE('2022-01-01','-','') AND REPLACE('2024-09-19','-','')
		AND A.CHADAE_DIV = '2'
		AND A.ACCO_CODE = '102'
		AND A.SLIP_YN = 'N'
		AND (D.SLIP_YN IS NULL OR D.SLIP_YN = 'N')  //이부분을 추가함
		AND (A.CONN_SLIP_NO = '' OR A.CONN_SLIP_NO1='' OR A.CONN_SLIP_NO2 = '')
	ORDER BY A.SLIP_NO DESC, A.SLIP_SEQ ASC

쿼리에서 해당 부분을 추가한 이유는 전표등록전에 연계처리를 한(TRSLTP의 SLIP_YN='Y')데이터는 미처리 연계팝업에서 보여주면 안됨.
수정 전에는 메시지로 이미 연계처리하고있는 전표입니다. 라고 뜨고있지만
분할에대해서는 임시테이블에서 SLIP_YN이 N이기때문에   
```

```
//위에서 띄운 미처리 전표 팝업에서 확인 클릭 시
function junpyoCreate() {
		var junpyoQcell = QCELL.getInstance('junpyoQcell'), 
		qcell = QCELL.getInstance('qcell'), 
		selectedRow = qcell.getIdx('row'),
		nRow = junpyoQcell.getIdx('row'), 
		nHeaderRow = junpyoQcell.attr('headerrows'), 
		chks = junpyoQcell.getRowheaderChecked(0); // 첫번째 열이 checkBox
		...
		/*1. 분할 시에(D) 입력한 처리금액과 체크한 금액과 비교**/
		if ($('input[name=dispose]:checked').val() == 'D') {
			for (var i = 0; i <= chks.length - 1; i++) { //체크한 잔액보다 입력한 처리금액이 큰 경우 
				if (junpyoQcell.getCellDataEx(chks[i], "balanceAmt") <= uncomma(disposeMoney)) {
					alert('분할처리금액은 미처리 잔액보다 크거나 같을 수 없습니다.')
					return false;
				}
		...
		/*1. 분할처리일때 input금액은 빈값이 될 수없다**/
		if ($('input[name=dispose]:checked').val() == 'D'
				&& document.getElementById('disposeMoney').value.replace(/\s| /gi, '') == '') {
			alert('분할 선택 시 처리금액은 빈값이 될 수 없습니다.')
			return false;
		}

		/*2. 임시테이블(TRSLTP)에 저장하기전에 중복 데이터 삭제**/
		//같은한줄에 대해서 겹치면 지운다.
		//지금은 connYn이 Y일때는 삭제 후 다시 생성하니까 안쓰인다.
		//deleteTrsltp(qcell.getCellDataEx(qcell.getIdx('row'),'connTime'))
		//임시테이블에 넣기
		for (var i = 0; i <= chks.length - 1; i++) {
			//위에서 deleteTrsltp는 내 userId,connTime이 같은걸 삭제
			//여기서는 다른 사람이랑 내가 연계할려는 항목이 같을때 TRSLTP에서 키인 SLIP_NO(미처리전표팝업), SLIP_SEQ(미처리전표팝업)가 임시테이블과 충돌하는지 확인
			if (checkTrsltpSlipMng( //받은 slipNo,slipSeq로 임시테이블에서 오로지 SLIP_NO, SLIP_SEQ만 비교해서 count를 가져온다.
					junpyoQcell.getCellDataEx(chks[i], "slipNo"), junpyoQcell.getCellDataEx(chks[i], "slipSeq")) != '0') {
				alert('전표번호[' + junpyoQcell.getCellDataEx(chks[i], "slipNo")+ ']는 이미연계처리 중입니다.')
				return false;
			}
		}
		//여기까지왔다면 임시테이블에 겹치는게 없다는 말이다.
		for (var i = 0; i <= chks.length - 1; i++) {
			if (chks.length > 1) { //미처리전표팝업에서 여러출 체크시
				if (i==0) { //첫줄은 이미 행추가시에 필요한값을 미리 넣었으니까 거래처코드만 가져온다.(즉, 확인누르면 체크한 미처리전표 팝업의 거래처를 전표그리드의 거래처에 넣음)
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'custCode1',junpyoQcell.getCellDataEx(chks[i], 'custCode'))
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'custName1',junpyoQcell.getCellDataEx(chks[i], 'custName'))
				}
				//미처리전표 팝업에서 두줄이상 체크시에만 행 추가 및 기본값세팅
				if (i > 0) { //첫줄은 행추가로 추가 및 값세팅이 되니까 아래 값세팅은 두번째줄부터 세팅해주면된다.
					qcell.insertRow(qcell.getIdx('row') + i)
					sleep(1000);//행추가시 시간이 겹치는 현상으로 인한 추가
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'custCode1',junpyoQcell.getCellDataEx(chks[i], 'custCode'))
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'custName1',junpyoQcell.getCellDataEx(chks[i], 'custName'))
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'slipDate', getToday2().replace(/-/g, ""))
				...
				junpyoQcell.setCellDataEx(chks[i], 'accoTime', qcell.getCellDataEx(qcell.getIdx('row') + i, 'connTime'))
				junpyoQcell.setCellDataEx(chks[i],'slipYn',$('input[name=dispose]:checked').val() == 'A' ? 'Y' : 'N')
				//여기 분기문은 여러개 체크를 했을 때니까 분할은 해당사항이 안되서 document.getElementById('disposeMoney').value
				//value 값을 구해서 junpyoQcell의 slipAmt에 넣을 필요가 없다. 
				//그냥 junpyoQcell목록에 나와있는 금액을 체크한 그대로 임시테이블에 넣으면 된다.
				insertTrsltp(junpyoQcell.getRowData(chks[i]));

				//전체이면서 차변일때(여러줄 체크시)
				if ($('input[name=dispose]:checked').val() == 'A' && qcell.getCellDataEx(qcell.getIdx('row'), 'chadaeDiv') == '1') {
					//여러출 체크했을 때는 합계금액이 필요없다.
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'debtorAmt',junpyoQcell.getCellDataEx(chks[i], 'balanceAmt'))
				} else if ($('input[name=dispose]:checked').val() == 'A' && qcell.getCellDataEx(qcell.getIdx('row'), 'chadaeDiv') == '2') { //전체일때	
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'creditAmt',junpyoQcell.getCellDataEx(chks[i], 'balanceAmt'))
				}

				//connYn에 대입
				if ($('input[name=dispose]:checked').val() == 'A'&& junpyoQcell.getCellDataEx(chks[i], 'balanceAmt') == 0) {
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'connYn', 'N') //처음에는 확인누를때는 Y 단, 넘어오는 금액이 0원이면 N처리
					//goToCell(qcell.getIdx('row'),3)
					//그외는 Y로 넣기
				} else {
					qcell.setCellDataEx(qcell.getIdx('row') + i, 'connYn', 'Y') //처음에는 확인누를때는 Y 단, 넘어오는 금액이 0원이면 N처리
					//goToCell(qcell.getIdx('row'),3)
				}
...
function settingCardBox() {}  - document.getElementById("selectCardBox")
...
<img src="https://github.com/seongdongjo/JavaStudy/blob/main/%EB%A1%9C%EC%A7%81%EC%A0%95%EB%A6%AC/3.PNG" width="600" height="100" /><br><br>
```
#)부대비용 조회 쿼리를 쪼개보자
...
SELECT 
	...
	A.CNTR_SEAL, 
	A.CNTR_SEAL2,
    CONCAT(A.CNTR_SZ,' / ',A.CNTR_TP,' / ',A.CNTR_FE) AS CNTR_SZ_TP, 
	IFNULL(A.CLAIM_AMT,0) AS CLAIM_AMT,
	IFNULL(B.PICKUP_CHARGE,0)  AS PICKUP_CHARGE,   IFNULL(B.PICKUP_TAXYN,'N')   AS PICKUP_TAXYN,
	IFNULL(B.STORAGE_CHARGE,0) AS STORAGE_CHARGE,  IFNULL(B.STORAGE_TAXYN,'N')  AS STORAGE_TAXYN,
	IFNULL(B.CANCEL_CHARGE,0)  AS CANCEL_CHARGE,   IFNULL(B.CANCEL_TAXYN,'N')   AS CANCEL_TAXYN,
	IFNULL(B.WAITING_CHARGE,0) AS WAITING_CHARGE,  IFNULL(B.WAITING_TAXYN,'N')  AS WAITING_TAXYN,
	IFNULL(B.LOADING_CHARGE,0) AS LOADING_CHARGE,  IFNULL(B.LOADING_TAXYN,'N')  AS LOADING_TAXYN,
	IFNULL(B.SHUTTLE_CHARGE,0) AS SHUTTLE_CHARGE,  IFNULL(B.SHUTTLE_TAXYN,'N')  AS SHUTTLE_TAXYN,
	IFNULL(B.W_SURCHARGE,0)    AS W_SURCHARGE,     IFNULL(B.W_SUR_TAXYN,'N')    AS W_SUR_TAXYN,
	IFNULL(B.C_SURCHARGE,0)    AS C_SURCHARGE,     IFNULL(B.C_SUR_TAXYN,'N')    AS C_SUR_TAXYN,
	IFNULL(B.TANK_SURCHARGE,0) AS TANK_SURCHARGE,  IFNULL(B.TANK_SUR_TAXYN,'N') AS TANK_SUR_TAXYN,
	IFNULL(B.H_SURCHARGE,0)    AS H_SURCHARGE,     IFNULL(B.H_SUR_TAXYN,'N')    AS H_SUR_TAXYN,
	IFNULL(B.REWORK_CHARGE,0)  AS REWORK_CHARGE,   IFNULL(B.REWORK_TAXYN,'N')   AS REWORK_TAXYN,
	IFNULL(B.EU_CHARGE,0)      AS EU_CHARGE,       IFNULL(B.EU_TAXYN,'N')       AS EU_TAXYN,
	IFNULL(B.CC_CHARGE,0)      AS CC_CHARGE,       IFNULL(B.CC_TAXYN,'N')       AS CC_TAXYN,
	IFNULL(B.OTHER_CHARGE,0)   AS OTHER_CHARGE,    IFNULL(B.OTHER_TAXYN,'N')    AS OTHER_TAXYN       
FROM TRORDT A 
